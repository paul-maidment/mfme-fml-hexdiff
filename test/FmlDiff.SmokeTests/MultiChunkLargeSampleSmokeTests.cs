...
public sealed class MultiChunkLargeSampleSmokeTests
{
    ...
    public void Multi_chunk_large_sample_decrypts_and_decompresses_to_expected_dat()
    {
        ...
        Assert.True(
            File.Exists(fmlPath),
            $"Smoke test FML not found at '{fmlPath}'. Set FMLDIFF_SMOKE_FML to override.");

        Assert.True(
            File.Exists(decodedPath),
            $"Smoke test decoded DAT not found at '{decodedPath}'. Set FMLDIFF_SMOKE_DECODED to override.");
